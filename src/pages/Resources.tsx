  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-text-primary font-semibold text-lg">Ressources</h1>
          <p className="text-text-secondary text-xs mt-0.5">
            {metrics_enabled
              ? `CPU, RAM et disques via SSH · rafraîchi toutes les ${metrics_interval_secs} s`
      ...
      {!metrics_enabled && (
        <div className="text-sm text-text-secondary bg-bg-tertiary border border-border-primary rounded-win p-4">
          Le monitoring des ressources est désactivé. Active-le dans{" "}
          <Link to="/settings" className="text-accent-primary hover:underline">Paramètres → Réseau</Link>.
        </div>
      )}
