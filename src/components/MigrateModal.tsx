...
export function MigrateModal({ vm, connectionId, onClose, onDone }: {
  ...
  useEffect(() => {
    invoke<MigrationPlan>("proxmox_migration_plan", { connectionId, node: vm.node, vmid: vm.vmid, vmType: vm.vm_type })
      .then((p) => { setPlan(p); setTarget(p.targets.find((t) => t.allowed)?.node ?? ""); })
  ...
  async function start() {
    ...
    try {
      ...
      const ok = exit === "OK";
      setPhase({ kind: "done", ok, message: ok ? `${vm.name} est maintenant sur ${target}` : `Échec : ${exit}` });
      if (ok) onDone();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-tertiary border border-border-primary rounded-win-lg shadow-win-hover w-full max-w-lg mx-4 animate-slide-in">
        <div className="flex items-center justify-between p-5 border-b border-border-primary">
          <h2 className="flex items-center gap-2 text-text-primary font-semibold"><ArrowRightLeft size={16} /> Migrer {vm.name}</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          {plan && (
            <>
              <p className="text-xs text-text-secondary">
                Depuis <span className="text-text-primary">{vm.node}</span> · {plan.running ? (vm.vm_type === "qemu" ? "migration à chaud" : "redémarrage sur la cible") : "invité arrêté"}
                {plan.local_disks.length > 0 && ` · disques : ${plan.local_disks.join(", ")}`}
              </p>
              {plan.notes.map((n) => (
                ...
                {plan.targets.map((t) => (
                  <label
                    ...
                    className={cn(
                      ...
                      t.allowed ? "cursor-pointer border-border-primary hover:bg-bg-hover" : "border-transparent opacity-70",
                      target === t.node && "border-accent-primary/50 bg-accent-primary/10"
                    )}
                  >
                    <input type="radio" name="target" disabled={!t.allowed || phase.kind === "running"} checked={target === t.node} onChange={() => setTarget(t.node)} className="mt-1 accent-accent-primary" />
                    <span className="flex-1">
                      <span className="flex items-center gap-2 text-text-primary">
                        {t.allowed ? <CheckCircle2 size={13} className="text-accent-success" /> : <XCircle size={13} className="text-accent-error" />}
                        {t.node}
                      </span>
                      {t.reasons.map((r) => <span key={r} className="block text-xs text-text-muted">{r}</span>)}
                    </span>
                  </label>
                ))}
              </div>
              {!plan.targets.some((t) => t.allowed) && (
              ...
              {phase.kind === "running" && <p className="flex items-center gap-2 text-sm text-text-secondary"><Loader2 size={14} className="animate-spin" /> Migration en cours… {phase.seconds} s</p>}
          ...
          <div className="flex gap-3 justify-end pt-2 border-t border-border-primary">
            <button onClick={onClose} disabled={phase.kind === "running"} className="px-4 py-2 text-sm rounded-win border border-border-primary text-text-secondary hover:bg-bg-hover disabled:opacity-50">
              {phase.kind === "done" ? "Fermer" : "Annuler"}
            </button>
            {phase.kind !== "done" && (
