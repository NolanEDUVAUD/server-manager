...
export function FilterBar({ scope, searchRef, placeholder, shown, total, onOrganise }: FilterBarProps) {
  ...
  const sortedFolders = [...folders].sort((a, b) => a.name.localeCompare(b.name, locale, { sensitivity: "base" }));
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[14rem]">
          ...
          />
          <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-text-muted border border-border-primary rounded px-1.5 py-0.5" title={t("filters.shortcut")}>/</kbd>
        </div>
        <select className={selectClass} value={f.status} onChange={(e) => setFilters(scope, { status: e.target.value as StatusFilter })} aria-label={t("filters.statusAria")}>
          <option value="all">{t("filters.allStatuses")}</option>
          <option value="online">{t("common.online")}</option>
          <option value="offline">{t("common.offline")}</option>
        </select>
        <select className={selectClass} value={f.folderId ?? ""} onChange={(e) => setFilters(scope, { folderId: e.target.value || null })} aria-label={t("filters.folderAria")}>
          <option value="">{t("filters.allFolders")}</option>
          <option value={NO_FOLDER}>{t("filters.noFolder")}</option>
        ...
        >
          <Star size={13} className={f.favoritesOnly ? "fill-current" : ""} /> {t("filters.favorites")}
        </button>
        <button
          type="button"
          onClick={onOrganise}
          ...
          title={t("filters.organiseTitle")}
        >
          <FolderOpen size={13} /> {t("filters.organise")}
        </button>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {tags.length > 0 && (
            <>
              ...
              {tags.map((tag) => <TagChip key={tag.id} tag={tag} active={f.tagIds.includes(tag.id)} onClick={() => toggleTag(tag.id)} />)}
              {f.tagIds.length > 1 && <span className="text-[11px] text-text-muted">{t("filters.allTags")}</span>}
            </>
          )}
          {active && (
...
export function NoFilterResults({ scope }: { scope: FilterScope }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-12 text-center">
      ...
      <div>
        <p className="text-sm text-text-primary font-medium">{t("filters.noResults")}</p>
        <p className="text-xs text-text-muted mt-0.5">{t("filters.noResultsHint")}</p>
      </div>
      <button
        type="button"
