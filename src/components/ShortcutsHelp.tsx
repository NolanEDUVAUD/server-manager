...
function groups(): { name: string; items: ShortcutDef[] }[] {
  const out: { name: string; items: ShortcutDef[] }[] = [];
    ...
    <span className="flex items-center gap-1 shrink-0">
      {steps.map((combo, i) => (
        <span key={i} className="flex items-center gap-1">
          {i > 0 && <span className="text-[11px] text-text-muted">puis</span>}
          {combo.map((k, j) => (
...
export function ShortcutsHelp() {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center">
      ...
      <div role="dialog" aria-label="Raccourcis clavier" className="relative bg-bg-tertiary border border-border-primary rounded-win-lg shadow-win-hover w-full max-w-xl mx-4 max-h-[85vh] flex flex-col animate-slide-in">
        <div className="flex items-center justify-between p-5 border-b border-border-primary">
          <h2 className="flex items-center gap-2 text-text-primary font-semibold"><Keyboard size={16} /> Raccourcis clavier</h2>
          <button onClick={() => setShortcutsHelpOpen(false)} className="text-text-secondary hover:text-text-primary" aria-label="Fermer l'aide"><X size={18} /></button>
        </div>
        <div className="p-5 space-y-5 overflow-y-auto">
          {groups().map((g) => (
            <section key={g.name}>
              <h3 className="text-xs font-medium text-text-secondary uppercase tracking-wide mb-2">{g.name}</h3>
          ...
          <p className="text-[11px] text-text-muted">
            Les raccourcis à une touche (« ? », « / », « g » puis une lettre) sont ignorés pendant la saisie dans un champ
            ou dans la console SSH.
          </p>
        </div>
      </div>
    </div>
